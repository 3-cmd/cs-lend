...
import com.cs.core.enums.ReturnMethodEnum;
...
public class LendServiceImpl extends ServiceImpl<LendMapper, Lend> implements LendService {
    ...
    @Override
    public Page<Lend> getByPageList(Long currentPage, Long pageSize, String keyword) {
        ...
        wrapper.like(keyword != null && !keyword.isEmpty(), Lend::getPeriod, keyword)
                .or()
                .like(keyword != null && !keyword.isEmpty(), Lend::getLendYearRate, keyword)
                .or()
                .like(keyword != null && !keyword.isEmpty(), Lend::getReturnMethod, keyword);
        //对数据的每一条进行处理,显示对应的文字信息
        Page<Lend> myPage = this.page(page, wrapper);
        List<Lend> lendList = myPage.getRecords().stream().peek(item -> {
            String returnMethod = dictService.getNameByParentDictCodeAndValue("returnMethod", item.getReturnMethod());
            item.getParam().put("status", LendStatusEnum.getMsgByStatus(item.getStatus()));
            item.getParam().put("returnMethod", returnMethod);
        }).collect(Collectors.toList());
        myPage.setRecords(lendList);
        return myPage;
    }
